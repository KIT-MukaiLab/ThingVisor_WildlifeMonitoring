...
class FetcherThread(Thread):    #Edites in 2 places
    """
    # Thread used to fetch and publish data
    def __init__(self): #Edites
        ...
        vThingID1 = v_thing_ID1

        ngsiLdEntity1 = {"id": "urn:ngsi-ld:" + location + ":" + sensor1,
                         "type": sensorType1,
                         app: {"type": "Property", "value": "data"}}

        context_hello.set_all([ngsiLdEntity1])  # set initial context for hello virtual thing
    """
    def run(self):      #Edites
        while True:
            location = "KIT"
...
class mqttDataThread(Thread):       #No Edites
...
class MqttControlThread(Thread):    #Edites in 3 places
    def on_message_get_thing_context(self, jres):           #Edites
        silo_id = jres["vSiloID"]
        message3 = {"command": "getContextResponse", 
                    "data": context_hello.get_all(), 
                    "meta": {"vThingID": v_thing_ID3}}
        mqtt_control_client.publish(v_silo_prefix + "/" + silo_id + "/" + in_control_suffix,
                                    str(message3).replace("\'", "\""))

    def send_destroy_v_thing_message(self):                 #Edites
        msg = {"command": "deleteVThing", "vThingID": v_thing_ID3, "vSiloID": "ALL"}
        mqtt_control_client.publish(v_thing_prefix + "/" + v_thing_ID3 + "/" + out_control_suffix,
                                    str(msg).replace("\'", "\""))
        return

    def send_destroy_thing_visor_ack_message(self):         #No Edites
    ...
    def on_message_destroy_thing_visor(self, jres):         #No Edites
        global db_client
        db_client.close()
    ...
    def __init__(self):                                     #No Edites
        Thread.__init__(self)

    def on_message_in_control_vThing(self, mosq, obj, msg): #No Edites
    ...
    def on_message_in_control_TV(self, mosq, obj, msg):     #No Edites
        payload = msg.payload.decode("utf-8", "ignore")
        print(msg.topic + " " + str(payload))
        jres = json.loads(payload.replace("\'", "\""))
        try:
